...
cdef extern from "flint_wrap.h":
    ...
    gr_srcptr gr_vec_entry_srcptr(gr_vec_t vec, slong i, gr_ctx_t ctx) noexcept
    ...
    int _gr_vec_addmul_scalar_fmpz(gr_ptr vec1, gr_srcptr vec2, slong len, const fmpz_t c, gr_ctx_t ctx) noexcept
    ...
    truth_t _gr_vec_contains(gr_srcptr vec, slong len, gr_srcptr x, gr_ctx_t ctx) noexcept
    truth_t gr_vec_contains(const gr_vec_t vec, gr_srcptr x, gr_ctx_t ctx); noexcept
    int _gr_vec_sort(gr_ptr vec, slong len, gr_ctx_t ctx) noexcept
    int gr_vec_sort(gr_vec_t dest, const gr_vec_t src, gr_ctx_t ctx) noexcept
    void _gr_vec_permute(gr_ptr vec, slong * perm, slong len, gr_ctx_t ctx) noexcept
    int gr_vec_permute(gr_vec_t dest, gr_vec_t src, slong * perm, gr_ctx_t ctx) noexcept
    void _gr_vec_shuffle(gr_ptr vec, flint_rand_t state, slong len, gr_ctx_t ctx) noexcept
